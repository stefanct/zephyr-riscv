...
static struct State sm2_idle = {.id_name = CYCLE_STATE_IDLE, .default_next_state = CYCLE_STATE_IDLE,
        .timing_goal_start = 0, .timing_goal_end = 0,  
        };

static struct State sm2_start = {.id_name = CYCLE_STATE_START, .default_next_state = CYCLE_STATE_DL,
        .timing_goal_start = 0, .timing_goal_end = 0,  
        };

static struct State sm2_dl = {.id_name = CYCLE_STATE_DL, .default_next_state = CYCLE_STATE_UL,
        .timing_goal_start = 0, .timing_goal_end = 0,  
        };

static struct State sm2_ul = {.id_name = CYCLE_STATE_UL, .default_next_state = CYCLE_STATE_RL,
        .timing_goal_start = 0, .timing_goal_end = 0,  
        };

static struct State sm2_rl = {.id_name = CYCLE_STATE_RL, .default_next_state = CYCLE_STATE_END,
        .timing_goal_start = 0, .timing_goal_end = 0,  
        };

static struct State sm2_end = {.id_name = CYCLE_STATE_END, .default_next_state = CYCLE_STATE_IDLE,
        .timing_goal_start = 0, .timing_goal_end = 0, 
        };
...
static void config_timing_goals(int period_irq1_us, int period_irq2_us, int num_substates){
    ...
    // todo: check!
    sm2_dl.timing_goal_start = 0;
    sm2_dl.timing_goal_end = t_state;

    sm2_ul.timing_goal_start = sm2_dl.timing_goal_end + t_safe_inter_states;
    // end of first substate
    sm2_ul.timing_goal_end = sm2_ul.timing_goal_start + t_substate;
    int t_state_ul = sm2_ul.timing_goal_end - sm2_ul.timing_goal_start;

    sm2_rl.timing_goal_start = sm2_ul.timing_goal_start + num_substates * (t_state_ul + t_safe_inter_states);
    sm2_rl.timing_goal_end = sm2_rl.timing_goal_start + t_state;

    sm2_end.timing_goal_end = sm2_rl.timing_goal_end + t_safe_inter_states;

    // make sure last end is < T(protocol cycle)
    if(sm2_end.timing_goal_end > period_irq1_cyc)
...
void sm2_print_state_config(){

    printk("SM2 State config: \n");

    for(int i=0; i<_NUM_CYCLE_STATES; i++){
        struct State * state_cur = &(sm2_states[i]);
        int j = 0;
        do{
...
void sm2_run(struct device * dev, int period_irq1_us, int period_irq2_us){
    ...
    states_configure_substates(&sm2_ul, num_substates, substate_summand);
    // transfer into and init state array
    sm2_states[0] = sm2_idle;
    sm2_states[1] = sm2_start;
    sm2_states[2] = sm2_dl;
    sm2_states[3] = sm2_ul;
    sm2_states[4] = sm2_rl;
    sm2_states[5] = sm2_end;
