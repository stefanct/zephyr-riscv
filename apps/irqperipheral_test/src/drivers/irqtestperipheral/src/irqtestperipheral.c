...
#define IRQT_FE310_NAME 				CONFIG_FE310_IRQT_NAME
#define IRQT_IRQ_0_PIN					CONFIG_FE310_IRQT_IRQ_0_PIN
#define IRQT_FAST_ID2IDX				CONFIG_FE310_IRQT_FAST_ID2IDX
#else
#define IRQT_IRQ_0_PIN 					-1
#endif
#define IRQT_0_IRQ_0             		(RISCV_MAX_GENERIC_IRQ + IRQT_IRQ_0_PIN)
#define IRQT_0_IRQ_1             		(RISCV_MAX_GENERIC_IRQ + IRQT_IRQ_0_PIN + 1)
#define IRQT_0_IRQ_2             		(RISCV_MAX_GENERIC_IRQ + IRQT_IRQ_0_PIN + 2)
 ...
 * 
 * Used if IRQT_FAST_ID2IDX > 1 set.
 ...
 * 
 * Used if IRQT_FAST_ID2IDX > 1 not set.
...
static inline irqt_irq_id_t _get_irq_id(){
	//return 0; // debug only
	return riscv_plic_get_irq() - IRQT_IRQ_0_PIN;
}
...
static void plic_fast_handler(int irq_num){
	...
	(( void(*)(void) )cbs_p[irq_num - IRQT_IRQ_0_PIN])();


	/*
	switch(irq_num){
		case IRQT_0_IRQ_0 - RISCV_MAX_GENERIC_IRQ:	
			cb_irq0();
			//data->cb_arr[0]();
			return;
		case IRQT_0_IRQ_1 - RISCV_MAX_GENERIC_IRQ:
			cb_irq1();
			//data->cb_arr[1]();
			return;
		case IRQT_0_IRQ_2 - RISCV_MAX_GENERIC_IRQ:
			cb_irq2();
			//data->cb_arr[2]();
			return;
		default:
...
int irqtester_fe310_register_callback(struct device *dev, irqt_irq_id_t irq_id, void (*cb)(void)){	
	...
	switch(irq_id){
		case IRQ_0:
			irq_enable(IRQT_0_IRQ_0);
			break;
		case IRQ_1:
			irq_enable(IRQT_0_IRQ_1);
			break;
		case IRQ_2:
			irq_enable(IRQT_0_IRQ_2);
			break;
		default:
...
int irqtester_fe310_unregister_callback(struct device *dev, irqt_irq_id_t irq_id){	
	...
	switch(irq_id){
		case IRQ_0:
			irq_disable(IRQT_0_IRQ_0);
			break;
		case IRQ_1:
			irq_disable(IRQT_0_IRQ_1);
			break;
		case IRQ_2:
			irq_disable(IRQT_0_IRQ_2);
			break;
		default:
			SYS_LOG_WRN("Unknown IRQ id: %i", irq_id);
			return 1;
...
DEVICE_AND_API_INIT(irqtester_fe310, IRQT_FE310_NAME,
